        ...
        <div class="card shadow-sm">
          ...
          <div class="card-body bg-light" style="max-height: 400px; overflow-y: auto;">
            @forelse($messages as $msg)
              <div class="mb-3">
                <strong>{{ $msg->user->name ?? 'Ẩn danh' }}</strong>
                <small class="text-muted">({{ $msg->created_at->format('H:i d/m/Y') }})</small><br>
                <div class="border rounded p-2 bg-white">{{ $msg->content }}</div>
              </div>
            @empty
          ...
          <div class="card-footer">
            <form method="POST" action="{{ route('teacher.classroom.chat.send', $classroom->id) }}">
              @csrf
